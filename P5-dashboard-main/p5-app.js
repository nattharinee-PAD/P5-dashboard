...
(function () {
  ...
  function renderTraining() {
    const D = getData();
    const t = D.training;
    const fallbackTraining = window.P5_TEST_DATA?.training || {};
    const pages = t.pages || fallbackTraining.pages || [];
    const requests = t.openRequests || fallbackTraining.openRequests || [];
    const readyCount = requests.filter(r => r.status === 'พร้อมเปิด').length;
    const waitingCount = requests.filter(r => r.status.includes('รอ')).length;
    const totalLearners = requests.reduce((sum, r) => sum + (r.learners || 0), 0);
    const totalBudget = requests.reduce((sum, r) => sum + (r.budget || 0), 0);
    const avgReady = requests.length ? Math.round(requests.reduce((sum, r) => sum + (r.readiness || 0), 0) / requests.length) : 0;
    return `
    <div class="page" id="page-training">
      <div class="training-workbench">
        <div class="training-page-map">
          ${pages.map(page => `
            <button class="training-page-pill ${page.id === '2.2' ? 'active' : ''}" type="button">
              <span class="page-id">${page.id}</span>
              <span>
                <strong>${page.title}</strong>
                <small>${page.goal}</small>
              </span>
              <em>${page.status === 'prototype' ? 'กำลังทำ' : 'ต่อไป'}</em>
            </button>
          `).join('')}
        </div>

        <div class="training-22-grid">
          <section class="card training-hero-card">
            <div class="card-head">
              <div>
                <h3>2.2 ขอเปิดอบรม ภายใน/ภายนอก</h3>
                <p>Training request cockpit · ใช้คุมคำขอก่อนส่งอนุมัติและเปิดรุ่นอบรม</p>
              </div>
              <div class="card-chip success">${readyCount} พร้อมเปิด</div>
            </div>
            <div class="request-kpis">
              ${requestKpi('คำขอทั้งหมด', requests.length, 'รายการ', 'purple')}
              ${requestKpi('รออนุมัติ/ตรวจงบ', waitingCount, 'รายการ', 'gold')}
              ${requestKpi('ผู้เรียนรวม', fmt(totalLearners), 'คน', 'blue')}
              ${requestKpi('งบทดสอบ', totalBudget ? fmt(totalBudget) : '0', 'บาท', 'green')}
              ${requestKpi('ความพร้อมเฉลี่ย', avgReady + '%', 'readiness', avgReady >= 80 ? 'green' : 'gold')}
            </div>
            <div class="request-toolbar">
              <button class="filter-chip active" type="button">ทั้งหมด</button>
              <button class="filter-chip" type="button">ภายใน</button>
              <button class="filter-chip" type="button">ภายนอก</button>
              <button class="filter-chip" type="button">รออนุมัติ</button>
              <button class="filter-chip" type="button">พร้อมเปิด</button>
              <button class="detail-toggle" type="button">+ สร้างคำขอ</button>
            </div>
            <div class="request-list">
              ${requests.map(renderTrainingRequestCard).join('')}
            </div>
          </section>

          <aside class="card request-side-panel">
            <div class="card-head compact">
              <div>
                <h3>มุมมองอนุมัติ</h3>
                <p>ลำดับงานที่ 2.2 ควรเห็นในหน้าเดียว</p>
              </div>
            </div>
            <div class="approval-flow">
              ${approvalStep('รับคำขอ', 'PAD ตรวจข้อมูลเบื้องต้น', 'done')}
              ${approvalStep('ตรวจความพร้อม', 'หลักสูตร/วิทยากร/รายชื่อ/ห้อง/งบ', 'active')}
              ${approvalStep('ส่งอนุมัติ', 'Manager หรือ HRBP ตามประเภทอบรม', 'pending')}
              ${approvalStep('เปิดรุ่นอบรม', 'สร้างรอบและแจ้งผู้เรียน', 'pending')}
            </div>
            <div class="side-divider"></div>
            <h4 class="mini-title">ข้อมูลที่ฟอร์ม 2.2 ต้องมี</h4>
            <div class="field-cloud">
              ${['ชื่อหลักสูตร', 'ประเภทอบรม', 'BU', 'วันที่', 'สถานที่', 'จำนวนผู้เรียน', 'วิทยากร', 'งบประมาณ', 'ผู้ขอ', 'วัตถุประสงค์'].map(item => `<span>${item}</span>`).join('')}
            </div>
            <div class="side-divider"></div>
            <h4 class="mini-title">กติกาแจ้งเตือน</h4>
            <ul class="compact-list">
              <li>Readiness ต่ำกว่า 60% ให้ติดป้ายแบบร่าง</li>
              <li>อบรมภายนอกต้องมีงบและ provider ก่อนส่งอนุมัติ</li>
              <li>รายการพร้อมเปิดต้องไม่มี blocker ค้าง</li>
            </ul>
          </aside>
        </div>
      </div>
  ...
  function requestKpi(label, value, unit, color) {
    return `
      <div class="request-kpi ${color}">
        <div class="request-kpi-label">${label}</div>
        <div class="request-kpi-value">${value}</div>
        <div class="request-kpi-unit">${unit}</div>
      </div>`;
  }

  function renderTrainingRequestCard(req) {
    const readinessCls = req.readiness >= 85 ? 'green' : req.readiness >= 60 ? 'gold' : 'red';
    const statusCls = req.status === 'พร้อมเปิด' ? 'success' : req.status === 'แบบร่าง' ? 'muted' : 'warn';
    return `
      <article class="request-card">
        <div class="request-main">
          ...
          <div class="request-meta">
            ...
            <span>${req.learners} คน</span>
            <span>${req.owner}</span>
          </div>
        </div>
        <div class="request-readiness">
          <div class="ring ${readinessCls}" style="--pct:${req.readiness}">
            <span>${req.readiness}%</span>
          </div>
          <small>พร้อมเปิด</small>
        </div>
        <div class="request-detail">
          <div><strong>วิทยากร</strong><span>${req.trainer}</span></div>
          <div><strong>สถานที่</strong><span>${req.venue}</span></div>
          <div><strong>งบ</strong><span>${req.budget ? fmt(req.budget) + ' บาท' : 'ไม่ใช้งบ'}</span></div>
          <div><strong>Blocker</strong><span>${req.blockers.length ? req.blockers.join(' · ') : 'ไม่มี'}</span></div>
        </div>
        <div class="request-checklist">
          ${req.checklist.map((item, idx) => `<span class="${idx < Math.round(req.readiness / 20) ? 'done' : ''}">${item}</span>`).join('')}
        </div>
      </article>`;
  }

  function approvalStep(title, text, state) {
    return `
      <div class="approval-step ${state}">
        <span></span>
        <div>
          <strong>${title}</strong>
          <small>${text}</small>
